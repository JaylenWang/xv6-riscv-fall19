...
bd_malloc(uint64 nbytes)
{
  ...
  for(; k > fk; k--) {
    ...
    bit_flip(bd_sizes[k-1].alloc, blk_index(k-1, p)>>1);
...
bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {   // skip max size
    ...
    if(bit_isset(bd_sizes[k].alloc, left>>1)) {
      free += BLK_SIZE(k);
      lst_push(&bd_sizes[k].free, addr(k, max(left, get_buddy(left))));
    }

    if(right <= left)
      continue;

    // free += bd_initfree_pair_r(k, right);
    if(bit_isset(bd_sizes[k].alloc, right>>1)) {
      free += BLK_SIZE(k);
      lst_push(&bd_sizes[k].free, addr(k, min(right, get_buddy(right))));
    }
  }
  return free;
}
